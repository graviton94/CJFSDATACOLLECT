...
class FDACollector:
    ...
    def fetch_index_page(self) -> List[Dict[str, str]]:
        ...
        if master_file.exists():
            print(f"📄 Loading alerts from master index: {master_file}")
            try:
                ...
                if not df.empty:
                    ...
                    print(f"✅ Loaded {len(alerts)} alerts from local index.")
                    return alerts
            except Exception as e:
                print(f"❌ Error reading parquet index: {e}")
        
        # 2. Fallback to Web Scraping (Legacy)
        print("⚠️ Master index not found/readable. Falling back to web scraping...")
        try:
            ...
            print(f"✅ Found {len(alerts)} Import Alerts from web")
    ...
    def collect(self, force_update: bool = True) -> pd.DataFrame:
        ...
        if not alerts:
            print("⚠️ No alerts found from indexer.")
        ...
        if len(df) < initial_count:
            print(f"   ℹ️  Dropped {initial_count - len(df)} duplicate records internally.")
            
        df = validate_schema(df)
        
        print(f"✅ Collected {len(df)} new records from {processed_count} alerts")
